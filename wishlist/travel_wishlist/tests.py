...
class TestVisitedPage(TestCase):
    ...
    def test_view_visited_page_shows_empty_list_message(self):
...
class TestVisitedPageEmptyList(TestCase):
    
    def test_visited_page_shows_message_for_empty_list(self):
        response = self.client.get(reverse('places_visited'))
        self.assertTemplateUsed(response, 'travel_wishlist/visited.html')
...
class TestVisitPlace(TestCase):
    ...
    def test_visit_non_existent_place(self):
        visit_nonexistent_url = reverse('place_was_visited', args=(123456, ))
        response = self.client.post(visit_nonexistent_url)
